...
import { Card, Button, Space, Select, Table, Typography, Input, message, Tag, Tree, Empty, Spin, Statistic, Row, Col } from 'antd'
...
export default function ManualBook() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return flatItems.filter((x) => {
      const s = [
        x?.machine_category, x?.machine, x?.problem_category,
        x?.problem, x?.root_cause, x?.solution,
      ].filter(Boolean).join(' ')
      return s.includes(q)
    })
  }, [flatItems, query])
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 240,
      render: (v) => (
        <div style={{ minWidth: 180, whiteSpace: 'normal', wordBreak: 'break-word', lineHeight: 1.6, color: '#1a1a1a' }}>
          {v || '-'}
        </div>
      ),
    },
    {
      ...
      width: 200,
      render: (v) => (
        <div style={{ minWidth: 140, whiteSpace: 'normal', wordBreak: 'break-word', lineHeight: 1.6, color: '#595959' }}>
          {v || '-'}
        </div>
      ),
    },
    {
      ...
      width: 240,
      render: (v) => (
        <div style={{ minWidth: 160, whiteSpace: 'normal', wordBreak: 'break-word', lineHeight: 1.6, color: '#389e0d' }}>
          {v || '-'}
        </div>
      ),
    },
