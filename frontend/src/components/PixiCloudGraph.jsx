...
const textUnits = (value) => Array.from(String(value || '')).reduce((sum, ch) => {
...
const estimateNodeHeight = (label, kind) => {
  ...
  const unitsPerLine = innerWidth / (fontSize * 0.92)
  const lines = Math.max(1, Math.ceil(textUnits(label) / Math.max(unitsPerLine, 1)))
  return Math.max(minHeight, Math.ceil(lines * fontSize * 1.42 + 18))
}
      ...
      >
        ...
        {layoutNodes.map(({ id, node, kind, width, nodeHeight, left, top, style, radius }, i) => {
          return (
            ...
            >
              <span style={{ display: 'block', wordBreak: 'break-word', whiteSpace: 'normal' }}>{node.data?.label}</span>
            </button>
          )
        })}
      </div>
